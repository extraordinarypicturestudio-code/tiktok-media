#!/usr/bin/env python3
"""
Publie la prochaine video "pending" d'une file d'attente sur un compte
YouTube (Shorts) via l'API Zernio, puis marque l'entree comme publiee dans
le meme fichier.

Copie adaptee de publish_next.py (TikTok) - meme logique de fiabilite
(retry, verification de statut, registre de hashs anti-doublon), seul le
format de la requete de publication change (platform "youtube",
platformSpecificData au lieu de tiktokSettings). Garde separee du script
TikTok pour ne pas risquer de casser un pipeline deja fiable.
...
Usage : python publish_next_youtube.py <queue.json> <pseudo_youtube>
"""
...
def enregistrer_et_supprimer(video, channel):
    ...
    fichier local. Meme registre partage que le pipeline TikTok, pour
    empecher de re-sourcer la meme video sur une autre chaine par erreur."""
...
def compte_id(pseudo_youtube):
    data = zernio_call("GET", "/accounts")
    items = data.get("accounts", data) if isinstance(data, dict) else data
    for a in items:
        if a.get("platform") == "youtube" and a.get("username") == pseudo_youtube:
            return a.get("_id") or a.get("id")
    raise RuntimeError(f"Compte YouTube '{pseudo_youtube}' introuvable dans Zernio")


def publier(account_id, url_video, legende, titre):
    payload = {
        "content": legende,
        "publishNow": True,
        "platforms": [{
...
def attendre_statut(post_id, delai_max=420, intervalle=20):
    ...
    Meme raisonnement que pour TikTok : l'API accepte la requete tout de
    suite mais YouTube peut refuser/retarder le traitement. Sans cette
    attente, un echec serait note comme publie et ne sortirait jamais.
        ...
        if dernier == "failed":
            ...
            for p in post.get("platforms", []):
                if p.get("errorMessage"):
                    ...
                    break
            return "failed", msg
        print(f"  statut '{dernier}', attente de la confirmation YouTube...")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: publish_next_youtube.py <queue.json> <pseudo_youtube>")
        sys.exit(1)
