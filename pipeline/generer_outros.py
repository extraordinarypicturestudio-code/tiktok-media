...
CHAINES = {
    ...
    # love_kitchen97 : rebrand de nextlevelplays88. Le handle TikTok a
    # change le 2026-08-21 (reconnexion Zernio faite le meme jour, le champ
    # username de GET /accounts est passe de nextlevelplays88 a
    # love_kitchen97). Meme cle Zernio (zernio2.env) que l'ancien compte.
    "lovekitchen":    {"pseudo": "love_kitchen97", "env": "zernio2.env", "texte": "Follow for more", "label": "love_kitchen97"},
    # esprit_libre manquait ici alors que sa chaine publie depuis le 19/08 :
    # l'outro existait dans outros/ mais n'aurait pas pu etre regeneree si
    # elle avait ete perdue - exactement l'incident des 06-09/08/2026.
    "espritlibre":    {"pseudo": "esprit.libre18",  "env": "zernio3.env", "texte": "Abonne-toi pour plus"},
    # mindshift716 remplace esprit.libre18 depuis le 2026-08-22. L'outro
    # presente dans outros/ au 13/08 n'avait ete faite ni par cet outil ni
    # dans cette mise en page : logo centre seul, sans appel a l'abonnement
    # et sans handle. Sur une chaine qui demarre a 0 abonne, c'est le seul
    # endroit de la video qui demande l'abonnement - on la regenere ici.
    # Compte connecte sur la cle 2, pas la 3.
    "mindshift":      {"pseudo": "mindshift716",   "env": "zernio2.env", "texte": "Abonne-toi pour plus",
                       "logo": "outros/logo_mindshift.png"},
    # argile.histoires : chaine poterie a voix narrative, connectee le
    # 2026-08-26 sur la QUATRIEME cle (avec darkness3038), pas la troisieme.
    "argile":         {"pseudo": "argile.histoires", "env": "zernio4.env",
                       "texte": "Abonne-toi pour plus"},
    "pisciniste":     {"pseudo": "le.pisciniste7", "env": "zernio5.env",
                       "texte": "Une autre histoire demain",
                       "logo": "channels/pisciniste/avatar_pisciniste.png"},
}
...
def construire(nom, conf):
    ...
    # L'avatar servi par l'API TikTok fait 168x168 : agrandi au diametre de
    # 400 px de la frame, un logo charge (baseline, sous-titre) part en
    # bouillie. Quand le fichier d'origine est disponible en local, on part
    # de lui - "logo" dans CHAINES - et on ne telecharge rien.
    source_locale = conf.get("logo")
    if source_locale:
        brut = os.path.join(ICI, source_locale)
        if not os.path.exists(brut):
            raise RuntimeError(f"logo local introuvable : {brut}")
    else:
    ...
    # Ne jamais supprimer un logo fourni en local : c'est un fichier source
    # du depot, pas un temporaire telecharge.
    temporaires = (cercle,) if source_locale else (brut, cercle)
