...
def gemini_call(image_path, prompt):
    ...
    # Un 429 ou un 503 n'est pas un verdict sur la video : c'est le service
    # qui ne repond pas. Sans reprise, la regle "un controle qui n'a pas pu
    # tourner n'autorise pas la publication" transforme chaque hoquet de
    # quota en "clip fautif", et `montage_compilation.py` memorise ce verdict
    # dans `_verdicts` — le clip ne sera plus jamais reexamine. Ecrit le
    # 2026-09-06, apres avoir vu quatre traitements se disputer le meme
    # quota gratuit.
    import time
    data = None
